...
function loadPlaylists() {
    ...
    playlists.forEach((playlist, index) => {
        ...
        li.innerHTML = `
            ...
            <div>
                <button class="remix-btn-classs" id="remix-btn-${index}">
                    <img class="remixIcon" src="RemixIcon.png">
                </button>
        ...
        // Event Listener für den Remix-Button hinzufügen
        const remixButton = li.querySelector(`#remix-btn-${index}`);
        let originalPlaylistOrder = null;

        remixButton.addEventListener('click', function (e) {
            e.stopPropagation(); // Verhindert, dass das Klick-Ereignis zur Playlist weitergegeben wird

            if (!originalPlaylistOrder) {
                // Speichere die Original-Reihenfolge der aktuellen Playlist
                originalPlaylistOrder = [...playlist.songs];

                // Remixe die Playlist
                remixPlaylist(playlist);

                // Ändere das Bild auf das aktive Bild
                this.querySelector('.remixIcon').src = 'ActiveRemix.png';
            } else {
                // Stelle die Original-Reihenfolge wieder her
                playlist.songs = [...originalPlaylistOrder];
                originalPlaylistOrder = null;

                // Lade die Playlists neu, um die Änderungen zu zeigen
                loadPlaylists();

                // Ändere das Bild auf das normale Bild
                this.querySelector('.remixIcon').src = 'RemixIcon.png';
            }
        });
...
function remixPlaylist(playlist) {
    if (playlist) {
...
async function startPlaylist() {
    // Stoppe das aktuelle Abspielen, falls es läuft
    if (!audioPlayer.paused) {
        audioPlayer.pause();
    }

    try {
        // Setze die neue Quelle und lade das Audio
        audioSource.src = currentPlaylist.songs[currentSongIndex].src;
